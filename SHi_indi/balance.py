...
class Balance():
    ...
    def ReceiveRTData(self, RealType):  # 현재 파이썬 지원 안되는 듯. 추후 작동시 로직 수정 필요
        if RealType == 'AE':
            DATA = {}
            DATA['계좌번호'] = self.IndiReal.dynamicCall("GetSingleData(int)", 0)
            DATA['상품구분'] = self.IndiReal.dynamicCall("GetSingleData(int)", 1)
            DATA['종목코드'] = self.IndiReal.dynamicCall("GetSingleData(int)", 2)
            DATA['종목명'] = self.IndiReal.dynamicCall("GetSingleData(int)", 3)
            DATA['계좌명'] = self.IndiReal.dynamicCall("GetSingleData(int)", 4)
            DATA['매수매도구분'] = self.IndiReal.dynamicCall("GetSingleData(int)", 5) # 01: 매도, 02: 매수
            DATA['당일잔고'] = self.IndiReal.dynamicCall("GetSingleData(int)", 6)
            DATA['평균단가'] = self.IndiReal.dynamicCall("GetSingleData(int)", 7)
            DATA['미체결수량'] = self.IndiReal.dynamicCall("GetSingleData(int)", 8)
            DATA['청산가능수량'] = self.IndiReal.dynamicCall("GetSingleData(int)", 9)
            DATA['총매입금액'] = self.IndiReal.dynamicCall("GetSingleData(int)", 10)
            DATA['평가금액'] = self.IndiReal.dynamicCall("GetSingleData(int)", 11)
            DATA['평가손익'] = self.IndiReal.dynamicCall("GetSingleData(int)", 12)
            DATA['손익률'] = self.IndiReal.dynamicCall("GetSingleData(int)", 13)
            DATA['수수료'] = self.IndiReal.dynamicCall("GetSingleData(int)", 14)
            DATA['세금'] = self.IndiReal.dynamicCall("GetSingleData(int)", 15)
            DATA['현재가'] = self.IndiReal.dynamicCall("GetSingleData(int)", 17)
            DATA['거래승수'] = self.IndiReal.dynamicCall("GetSingleData(int)", 23)
            DATA['종목구분'] = self.IndiReal.dynamicCall("GetSingleData(int)", 24)    # 1: 선물, 2: 지수옵션, 3: 주식옵션
            DATA['종목매매손익'] = self.IndiReal.dynamicCall("GetSingleData(int)", 25)
            DATA['선물총매매손익'] = self.IndiReal.dynamicCall("GetSingleData(int)", 26)
            DATA['옵션총매매손익'] = self.IndiReal.dynamicCall("GetSingleData(int)", 27)
            DATA['기초자산명'] = self.IndiReal.dynamicCall("GetSingleData(int)", 28)
            DATA['이동평균단가'] = self.IndiReal.dynamicCall("GetSingleData(int)", 29)
            DATA['원체결단가'] = self.IndiReal.dynamicCall("GetSingleData(int)", 30)
            DATA['이동평균 매매손익'] = self.IndiReal.dynamicCall("GetSingleData(int)", 31)
