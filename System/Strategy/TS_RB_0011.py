...
class TS_RB_0011():
    ...
    def common(self):
        # Data load & apply
        self.lstData[self.ix] = Strategy.getHistData(self.lstProductCode[self.ix], self.lstAssetType[self.ix], self.lstTimeFrame[self.ix], 100)
    ...
    def applyChart(self):   # Strategy apply on historical chart
        ...
        for i in df.index-1:
            ...
            if self.fSL != 0.0: # Stop loss
                if (df['MP'][i-1] == 1) and (df['MP'][i] == 1):  # Exit long
                    if df['저가'][i-1] <= self.fSL:
                        df.loc[i, 'MP'] = 0
                        self.fSL = 0.0
                if (df['MP'][i-1] == -1) and (df['MP'][i] == -1): # Exit short
                    if df['고가'][i] >= self.fSL:
                        df.loc[i, 'MP'] = 0
                        self.fSL = 0.0
