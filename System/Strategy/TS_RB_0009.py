...
class TS_RB_0009():
    ...
    def common(self):
        # Data load & apply
        self.lstData[self.ix] = Strategy.getHistData(self.lstProductCode[self.ix], self.lstAssetType[self.ix], self.lstTimeFrame[self.ix], self.nP*2)
    ...
    def applyChart(self):   # Strategy apply on historical chart
        ...
        dfTemp['high_low'] = df['고가'] - df['저가']
        dfTemp['high_close'] = np.abs(df['고가'] - df['종가'].shift())
        dfTemp['low_close'] = np.abs(df['저가'] - df['종가'].shift())
        dfTemp['TR'] = dfTemp[['high_low', 'high_close', 'low_close']].max(axis=1)
        dfTemp['ATR'] = dfTemp['TR'].ewm(span=self.nP).mean()
        df[strColName2] = df[strColName1] + self.nMulti * dfTemp['ATR']
        df[strColName3] = df[strColName1] - self.nMulti * dfTemp['ATR']
        df['MP'] = 0
        for i in df.index:
            if i < self.nP:
                continue
