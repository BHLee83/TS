...
class TS_RB_0002():
    ...
    def common(self):
        # Data load & apply
        self.lstData[self.ix] = Strategy.getHistData(self.lstProductCode[self.ix], self.lstAssetType[self.ix], self.lstTimeFrame[self.ix], 25)
    ...
    def applyChart(self):   # Strategy apply on historical chart
        ...
        for i in df.index:
            ...
            # Entry
            nLast_month = df['일자'][i-1][4:6]
            nCurrent_month = df['일자'][i][4:6]
            if nCurrent_month != nLast_month:    # 월 변경시
                lstMonth_close.append(df['종가'][i-1])
