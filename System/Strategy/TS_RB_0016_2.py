...
class TS_RB_0016_2():
    ...
    def applyChart(self):   # Strategy apply on historical chart
        ...
        tmpRange = df['고가'] - df['저가']
        df['Range'] = tmpRange.rolling(window=4).mean()
            ...
            if i > self.nP2:
                ...
                if t < 1505:    # Entry 2
                    if (df['MP'][i-1] != 1) and (df['MP'][i] == 1):
                        self.fSellPrice = df['저가'][i] - df['Range'][i] * self.fMult
                    if (df['MP'][i-1] != -1) and (df['MP'][i] == -1):
                        self.fBuyPrice = df['고가'][i] + df['Range'][i] * self.fMult
                    if df['MP'][i] == 1:
                        if df['저가'][i] <= self.fSellPrice:
                            df.loc[i, 'MP'] = -1
                            self.strEntryName = 'RS'
                            self.fSellPrice += (df['저가'[i] - self.fSellPrice]) / self.nDiv
                    if df['MP'][i] == -1:
                        if df['저가'][i] >= self.fBuyPrice:
                            df.loc[i, 'MP'] = 1
                            self.strEntryName = 'RB'
                            self.fBuyPrice += (self.fBuyPrice - df['고가'[i]]) / self.nDiv

                if self.strEntryName == 'RB':   # Exit
                    if df['저가'][i] <= min(df['저가'][i-30:i]):
                        df.loc[i, 'MP'] = 0
                        self.strEntryName = ''
                if self.strEntryName == 'RS':
                    if df['고가'][i] >= max(df['고가'][i-30:i]):
                        df.loc[i, 'MP'] = 0
                        self.strEntryName = ''
        ...
        if type(PriceInfo) == int:  # 최초 실행
            ...
            if self.npPriceInfo == None:    # 첫 데이터 수신시
                ...
                if (str(self.npPriceInfo['체결시간'])[4:6] != str(PriceInfo['체결시간'])[4:6]) and \
                    ...
                    if int(str(PriceInfo['체결시간'])[2:4]) < 12: # Entry 1
                ...
                if int(str(PriceInfo['체결시간'])[2:4]) < 15: # Entry 2
                    if df['MP'][0] == 1:
                    ...
                    if df['MP'][0] == -1:
                        if (self.npPriceInfo['현재가'] < self.fBuyPrice) and (PriceInfo['현재가'] >= self.fBuyPrice):
                            Strategy.setOrder(self, self.lstProductCode[self.ix], 'B', self.amt_entry, PriceInfo['현재가'])
                            df.loc[0, 'MP'] = 1
                            self.logger.info('Buy %s amount ordered', self.amt_entry)
                            self.strEntryName = 'RB'

                if self.strEntryName == 'RB':
                    if (self.npPriceInfo['현재가'] > min(df['저가'][0:30])) and (PriceInfo['현재가'] <= min(df['저가'][0:30])):
                        ...
                        self.strEntryName = ''
                if self.strEntryName == 'RS':
                    if (self.npPriceInfo['현재가'] < max(df['고가'][0:30])) and (PriceInfo['현재가'] >= max(df['고가'][0:30])):
