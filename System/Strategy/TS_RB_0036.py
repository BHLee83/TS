        ...
        if self.fStopPrice != 0.0:  # Parabolic stop (execution)
            if (df.iloc[-1]['MP'] == 1) and (df.iloc[-2]['MP'] == 1):
                if (self.npPriceInfo['현재가'] >= self.fStopPrice) and (PriceInfo['현재가'] <= self.fStopPrice):
                    Strategy.setOrder(self.strName, self.lstProductCode[self.ix], 'EL', self.amt_entry, PriceInfo['현재가'])
                    df.loc[len(df)-1, 'MP'] = 0
                    self.fStopPrice = 0.0
                    self.logger.info('ExitLong %s amount ordered', self.amt_entry)
            if(df.iloc[-1]['MP'] == -1) and (df.iloc[-2]['MP'] == -1):
                if (self.npPriceInfo['현재가'] <= self.fStopPrice) and (PriceInfo['현재가'] >= self.fStopPrice):
                    Strategy.setOrder(self.strName, self.lstProductCode[self.ix], 'ES', self.amt_entry, PriceInfo['현재가'])
                    df.loc[len(df)-1, 'MP'] = 0
                    self.fStopPrice = 0.0
